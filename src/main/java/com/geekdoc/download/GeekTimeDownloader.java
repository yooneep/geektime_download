...
//        getColumn("100309001");
        ...
        File[] files = new File(filePath+ "md").listFiles();
...
//                .filter(p -> p.compareTo("100781001") >= 0)
                .forEach(cid -> getColumn(cid,files));

//        for (int i = 1000019; i < 1008198; i++) {
//            getColumnInfo(String.valueOf(i) + "01");
//        }
...
//        List<String> diffList = lessonList.stream()
//                .filter(item -> !cidList.contains(item))
//                .collect(Collectors.toList());
//        diffList.stream()
//                .sorted(Comparator.reverseOrder())
////                .filter(p -> p.compareTo("100006101") < 0)
//                .forEach(p -> ThreadUtil.execute(()->getColumn(p)));
//        System.out.println(diffList);
    ...
    private static void getColumnInfo(String cid) {
        Map data = new HashMap();
        data.put("product_id", Integer.valueOf(cid));
        data.put("with_recommend_article", true);
        String result = geekRequest(infoUrl, data);
        JSONObject jsonObject = JSONObject.parseObject(result);
        JSONObject dataJSON = jsonObject.getJSONObject("data");

        JSONObject article = dataJSON.getJSONObject("article");
        if (article == null) {
            return;
        }
        Integer count = article.getInteger("count");
        if (count < 3) {
            return;
        }
        boolean isVideo = dataJSON.getBoolean("is_video");
        if (isVideo) {
            return;
        }
        String title = dataJSON.getString("title");
        System.out.println("cid:" + cid + " title:" + title + " count:" + count);
    }


    // 下载指定专栏【NEW】
    private static void getColumn(String cid,File[] files) {
        // 获取专栏信息
        ...
        String mp3Path = filePath + "mp3" + File.separator + cid + "_" + info;
        String mp3urlPath = filePath + "url" + File.separator + cid + "_" + info;
        for (File file : files) {
...
//        if (FileUtil.isExist(mp3urlPath, mp3) && FileUtil.readFile(mp3urlPath,mp3)>1) {
//            log.info(info + " mp3齐全，跳过内容");
//            return;
//        } else {
//            for (int i = 0; i < list.size(); i++) {
//                String mp3Info = getMp3Info((JSONObject) list.get(i));
//                stringBuilder.append(mp3Info).append("\n");
//            }
//            FileUtil.write(stringBuilder.toString(), mp3urlPath, mp3);
//        }
        for (int i = 0; i < list.size(); i++) {
    ...
    private static String getMp3Info(JSONObject obj) {
        ...
        if(StringUtils.isBlank(audio_download_url)){
            log.error(article_title+ "audio_download_url is null");
        }
        return article_title + " url "+audio_download_url;
    }
    ...
    private static void saveArticle(JSONObject obj, String mdPath, String mp3Path, int i, boolean needmp3, Map<String, String> idTitleMap) {
        try {
            ...
            if (!FileUtil.isExist(mdPath, fileName)) {
                ...
                ThreadUtil.execute(() -> Html2MDUtil.convert(getArticle(id), mdPath + File.separator + finalFileName, true));
            }
    ...
    private static String getArticle(String id) {
        ...
        return "<p>" + stringBuilder.toString() + "</p>" +"<h2><strong>正文</strong></h2>"+ article_content;
    }

...
//        listFoldersAndFileCount("/Users/pangning/geektime/md/");
    ...
    public static void articles1(String cid) {
        Map data = new HashMap();
        data.put("cid", cid);
        data.put("order", "earliest");
        data.put("prev", 0);
        ...
        Arrays.stream(new File(filePath + File.separator + "md").listFiles()).filter(file -> file.getName().contains(info)).findFirst().ifPresent(file -> {
            File newFile = new File(mdPath);
            if (!file.renameTo(newFile)) {
                log.error("Failed to rename file: {}", file.getName());
            }
        });

        String head = getFileName("000__目录");
        if (FileUtil.isExist(mdPath, head) && !needmp3) {
            File[] files;
            try {
                files = new File(mdPath).listFiles();
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
            if (files.length == list.size() + 1) {
                log.info(info + " 文档齐全，跳过内容");
                return;
            }
        } else {
        ...
        for (int i = 0; i < list.size(); i++) {
            saveMarkDown((JSONObject) list.get(i), mdPath, mp3Path, i, needmp3);
        }
        log.info("完成!");
    }

    private static void saveMarkDown(JSONObject obj, String mdPath, String mp3Path, int i, boolean needmp3) {
        try {
            ...
            if (!FileUtil.isExist(mdPath, fileName)) {
                ...
                ThreadUtil.execute(() -> Html2MDUtil.convert(article, mdPath + File.separator + fileName, true));
            }
